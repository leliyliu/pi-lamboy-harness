...
- **pi-multiloop install guard** — `pi install … pi-multiloop` (bash) and write/edit of `~/.pi/agent/settings.json` introducing the package are denied unconditionally: its `get_goal`/`update_goal` collide with harness Goal tools — a tool-name conflict is a fatal pi startup error (verified pi 0.85.1). The deny reason steers the model to the deployed alternative (goal + parallel subagents + `bench_run`/`metric_compare`). Ordinary files mentioning the package are untouched.
